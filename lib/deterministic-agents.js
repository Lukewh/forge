...
const FORGE_BODY_SENTINEL = "<!-- forge-end -->";

/** Append sentinel to generated body so user additions after it are preserved on update. */
function withSentinel(body) {
  const trimmed = body.trimEnd();
  return trimmed.includes(FORGE_BODY_SENTINEL) ? trimmed : `${trimmed}\n\n${FORGE_BODY_SENTINEL}`;
}

/** Merge generated body with any user-added content that follows the sentinel. */
function mergeBody(generatedBody, existingBody) {
  const sentinelIdx = (existingBody || "").indexOf(FORGE_BODY_SENTINEL);
  if (sentinelIdx === -1) return withSentinel(generatedBody); // no sentinel yet — full replace
  const userContent = existingBody.slice(sentinelIdx + FORGE_BODY_SENTINEL.length).trimEnd();
  const base = withSentinel(generatedBody);
...
function upsertPullRequestRest(cwd, { base, branch, title, body }) {
  ...
  if (existing[0]?.number) {
    // Preserve any user-added content after the sentinel in the existing body
    const mergedBody = mergeBody(body, existing[0].body || "");
    const payload = { title, body: mergedBody };
  ...
  // New PR — add sentinel to body so future updates can preserve user additions
  const payload = { title, body: withSentinel(body), base, head };
  let input = writeTempJson(payload);
  try {
